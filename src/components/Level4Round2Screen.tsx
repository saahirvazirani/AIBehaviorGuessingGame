...
interface Level4Round2ScreenProps {
  onNextLevelClick: (wasCorrect: boolean) => void;
  round: number;
}

const Level4Round2Screen: React.FC<Level4Round2ScreenProps> = ({ onNextLevelClick, round }) => {
  const [answer, setAnswer] = useState<null | boolean>(null);
  const [answered, setAnswered] = useState(false);

  const correctAnswer = true; // "Yes" is the correct answer for Level 4, Round 2
  ...
  return (
    <div className={styles.container}>
      <h1 className={styles.title}>AI Behavior Guessing Game: Level 4, Round {round}</h1>
...
export default Level4Round2Screen;
